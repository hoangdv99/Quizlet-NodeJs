...
module.exports = {
	...
	fn: function(inputs, exits) {
		...
		if (req.signedCookies != undefined) {
			...
			return jwt.verify(req.signedCookies.sailsjwt, globals.jwtSecret, async function(err, payload) {
				...
				var user = await sails.models.user.findOne(payload.user)
		...
		if (req.headers != undefined) {
			...
			// if there is something, attempt to parse it as a JWT token
			return jwt.verify(token, globals.jwtSecret, async function(err, payload) {
				if (err || !payload.user) return exits.invalid()
				var user = await sails.models.user.findOne(payload.user)
				if (!user) return exits.invalid()
